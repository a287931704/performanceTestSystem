...
CREATE TABLE `test_stress_case_file` (
  ...
  `report_status` tinyint NOT NULL DEFAULT 1 COMMENT '状态  0：保存测试报告原始文件  1：不需要测试报告',
  `webchart_status` tinyint NOT NULL DEFAULT 1 COMMENT '状态  0：需要前端监控  1：不需要前端监控',
...
CREATE TABLE `test_stress_slave` (
  ...
  `case_id` bigint(20) COMMENT '用例ID',
...
INSERT INTO `test_stress_slave` (`slave_id`, `slave_name`, `case_id`, `ip`, `jmeter_port`, `user_name`, `passwd`, `ssh_port`, `home_dir`, `status`, `add_time`, `add_by`, `update_time`, `update_by`) VALUES ('1', 'LocalHost', NULL , '127.0.0.1', '1099', NULL, NULL, '22', '', '0', '2018-06-18 18:18:18', NULL, '2018-06-18 18:18:18', NULL);

-- 数据库中配置性能压测配置信息。key不要变。
INSERT INTO `sys_config` (`id`, `key`, `value`, `status`, `remark`) VALUES ('2', 'MASTER_JMETER_HOME_KEY', '/home/vmuser/jm/apache-jmeter-4.0', '1', '本地Jmeter_home绝对路径（Jmeter版本不要高于引擎版本）');
INSERT INTO `sys_config` (`id`, `key`, `value`, `status`, `remark`) VALUES ('3', 'MASTER_JMETER_CASES_HOME_KEY', '/home/vmuser/stressTestCases', '1', '本地保存用例数据的绝对路径，不要随意切换会导致文件找不到错误。');
...
INSERT INTO `sys_config` (`id`, `key`, `value`, `status`, `remark`) VALUES ('7', 'JMETER_THREADGROUP_SET_KEY', 'true', '1', 'true：开启线程组管理功能，上传脚本时线程组配置将入库管理，默认false。');
...
INSERT INTO `sys_config` (`id`, `key`, `value`, `status`, `remark`) VALUES ('9', 'SCRIPT_EXECUTE_FILE_HOME_KEY', '/service/script', '1', '执行外部python脚本目录');
