...
//    }
     ...
     */
    @SysLog("将参数化文件同步到指定分布式slave节点机的指定目录")
    @RequestMapping("/synchronizeFileBySplit")
    @RequiresPermissions("test:stress:synchronizeFile")
    public R synchronizeFileBySplit(@RequestBody Long[] fileIds) {
        stressTestFileService.synchronizeFileBySplit(fileIds);
        return R.ok();
    }
    ...
    public ResponseEntity<InputStreamResource> downloadFile(@PathVariable("fileId") Long fileId) throws IOException {
        ...
        headers.add("Content-Disposition",
                "attachment;filename=" + URLEncoder.encode(stressTestFile.getOriginName(), "UTF-8"));
