...
import io.renren.modules.test.utils.FileSplitUtils;
...
//        new JavassistEngine().fixJmeterStandrdEngine();
    ...
    @Override
    @Transactional
    public String run2(Long fileId, Long caseId) {
        return runByOne(fileId, caseId);
    }
     ...
     * 同理，也不会回滚这一事务。
     */
    public String runByOne(Long fileId, Long caseId) {
        StressTestFileEntity stressTestFile = queryObject(fileId);
        if (StressTestUtils.RUNNING.equals(stressTestFile.getStatus())) {
            throw new RRException("脚本正在运行");
        }
        ...
        String slaveStr = getSlaveIPPortByCaseId(caseId);
        // slaveStr用来做脚本是否是分布式执行的判断，不入库。
        stressTestFile.setSlaveStr(slaveStr);
    ...
    @Override
    @Transactional
    public void synchronizeFileBySplit(Long[] fileIds) {
        for (Long fileId : fileIds) {
            StressTestFileEntity stressTestFile = queryObject(fileId);
            //当前是向所有的分布式节点推送这个，阻塞操作+轮询，并非多线程，因为本地同步网卡会是瓶颈。
            Map query = new HashMap<>();
            query.put("status", StressTestUtils.ENABLE);
            query.put("caseId", stressTestFile.getCaseId());
            List<StressTestSlaveEntity> stressTestSlaveList = stressTestSlaveDao.queryList(query);
            //使用for循环传统写法
            //采用了先给同一个节点机传送多个文件的方式，因为数据库的连接消耗优于节点机的链接消耗
            String casePath = stressTestUtils.getCasePath();
            ...
            String targetPath = "";
            int rowCount = FileSplitUtils.getFileRowCount(filePath);
//            int rowCount = 4;
            if(fileNameSave.indexOf("\\") > 0){
                targetPath= casePath + File.separator + fileNameSave.split("\\\\")[0];
            }else{
                targetPath = casePath + File.separator + fileNameSave.split("/")[0];
            }
            FileSplitUtils.splitDataToSaveFile(rowCount / stressTestSlaveList.size(), new File(filePath), targetPath);
            int copyCount=1;
            for (StressTestSlaveEntity slave : stressTestSlaveList) {
                ...
                //StressTestFileEntity stressTestFile = queryObject(fileId);
                logger.debug("开始调putFileToSlaveBySplit，" + targetPath + File.separator + copyCount + File.separator + stressTestFile.getOriginName());
                putFileToSlaveBySplit(slave, ssh2Util, stressTestFile, targetPath + File.separator + copyCount + File.separator + stressTestFile.getOriginName());
     ...
     * 将文件上传到节点机目录上。
     */
    public void putFileToSlaveBySplit(StressTestSlaveEntity slave, SSH2Utils ssh2Util, StressTestFileEntity stressTestFile, String filePath) {
        String casePath = stressTestUtils.getCasePath();
        String fileNameSave = stressTestFile.getFileName();
//        String filePath = casePath + File.separator + fileNameSave;
//        fileSplitUtils.splitDataToSaveFile(100,stressTestFile.getFileName(), filePath);
//        String fileSaveMD5 = "";
//        try {
//            fileSaveMD5 = stressTestUtils.getMd5ByFile(filePath);
//        } catch (IOException e) {
...
//        }
        ...
        String caseFileHome = slave.getHomeDir() + "/bin/stressTestCases";
        logger.debug("caseFileHome----" + caseFileHome);
//        String MD5 = ssh2Util.runCommand("md5sum " + getSlaveFileName(stressTestFile, slave) + "|cut -d ' ' -f1");
//        if (fileSaveMD5.equals(MD5)) {//说明目标服务器已经存在相同文件不再重复上传
//            return;
//        }
...
//            newStressTestFile.setFileMd5(fileSaveMD5);
    ...
    public String getSlaveIPPortByCaseId(Long caseId) {
        Map query = new HashMap<>();
        query.put("status", StressTestUtils.ENABLE);
        query.put("caseId", caseId.toString());
        List<StressTestSlaveEntity> stressTestSlaveList = stressTestSlaveDao.queryList(query);
