$(function () {
    ...
    $("#jqGrid").jqGrid({
        ...
        colModel: [
            ...
            {
            	label: '执行操作', name: '', width: 130, sortable: false, formatter: function (value, options, row) {
                    var btn = '';
                    if (!(getExtension(row.originName) && /^(jmx)$/.test(getExtension(row.originName).toLowerCase()))) {
                        btn = "<a href='#' class='btn btn-primary' onclick='synchronizeFile(" + row.fileId + ")' ><i class='fa fa-arrow-circle-right'></i>&nbsp;同步文件</a>";
                        btn1 = "&nbsp;&nbsp;<a href='#' class='btn btn-primary' onclick='synchronizeFileBySplit(" + row.fileId + ")' ><i class='fa fa-arrow-circle-right'></i>&nbsp;分割同步</a>";
                        btn  = btn + btn1;
                    } else {
                        if (row.status == 1) {
                            btn = "<a href='#' class='btn btn-danger' onclick='stopOnce(" + row.fileId + ")' ><i class='fa fa-stop-circle'></i>&nbsp;停止</a>";
                        } else {
                            btn = "<a href='#' class='btn btn-primary' onclick='runByOne(" + row.fileId + "," + row.caseId +")' ><i class='fa fa-arrow-circle-right'></i>&nbsp;启动</a>";
                        }
                    }
...
function runByOne(fileId, caseId) {
    if (!fileId) {
        return;
    }
    $.ajax({
        ...
        url: baseURL + "test/stressFile/runOnce",
        contentType: "application/json",
        data: JSON.stringify({"fileId":fileId, "caseId":caseId}),
        success: function (r) {
            if (r.code == 0) {
                vm.reload();
                alert(r.msg, function () {
...
function synchronizeFileBySplit(fileIds) {
    if (!fileIds) {
        return;
    }
    confirm('确定向所有"启用的"分布式节点机推送该文件？文件越大同步时间越长', function () {
        $.ajax({
            type: "POST",
            url: baseURL + "test/stressFile/synchronizeFileBySplit",
            contentType: "application/json",
            data: JSON.stringify(numberToArray(fileIds)),
            success: function (r) {
                if (r.code == 0) {
